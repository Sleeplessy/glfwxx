...
#include <cassert>
glfw::window::window(int width, int height, std::string title) : raw_handle{glfwCreateWindow(width,height,title.c_str(), nullptr, nullptr)} , title(title){
...
glfw::window_id_t glfw::window_manager::create_window(int width, int height, std::string title,const std::function<void(window_ptr_t)>& callback) {
    glfw::window_id_t id = get_pool().size() + 1;
